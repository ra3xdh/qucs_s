...
Filter::Filter(Filter::FilterFunc ffunc_, Filter::FType type_, FilterParam par)
{
    ...
    if ((ftype == Filter::HighPass) || (ftype == Filter::LowPass)) {
        ...
        BW = fabs(Fu - Fl);
        F0 = sqrt(Fu * Fl);
        if ((ftype == Filter::BandPass) || (ftype == Filter::BandStop)) { // BandPass
...
void Filter::createPartList(QStringList& lst)
{
    ...
    lst << QStringLiteral("%1%2%3%4%5%6%7%8%9").arg("Stage#", 6).arg("C1", 12).arg("C2", 12).arg("R1(kOhm)", 10).arg("R2(kOhm)", 10).arg("R3(kOhm)", 10).arg("R4(kOhm)", 10).arg("R5(kOhm)", 10).arg("R6(kOhm)", 10);
...
bool Filter::checkRCL()
{
    for (auto& sec : Sections) {
        if (std::isnan(sec.R1) || std::isnan(sec.R2) || std::isnan(sec.R3) || std::isnan(sec.R4) || std::isnan(sec.C1) || std::isnan(sec.C2)) {
            return false;
        }
    }
    return true;
