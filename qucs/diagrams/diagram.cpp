...
int Graph::loadDatFile(const QString& fileName)
{
    ...
    if (g->lastLoaded.isValid())
        if (g->lastLoaded.toMSecsSinceEpoch() >= Info.lastModified().toMSecsSinceEpoch()) // Millisecond resulution is needed for tuning
...
QString Diagram::save()
{
    ...
    s += QString::number(rotX) + " " + QString::number(rotY) + " " + QString::number(rotZ);
