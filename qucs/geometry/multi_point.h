...
template <qucs_s::geom::PointLike P>
inline constexpr auto get_x(P&& point)
{
...
template <qucs_s::geom::PointLike P>
inline constexpr auto get_y(P&& point)
{
    if constexpr (std::is_pointer_v<std::decay_t<P>>) {
        return point->y();
    } else {
...
constexpr bool is_near_line(P1&& point_m, P2&& point_a, P3&& point_b, int dist)
{
    ...
    // horizontal line
    if (ay == by) {
        return gt_lt(get_y(point_m), std::pair { ay - dist, ay + dist }) && gt_lt(get_x(point_m), std::minmax(ax, bx));
    }

    // vertical line
    if (ax == bx) {
        return gt_lt(get_x(point_m), std::pair { ax - dist, ax + dist }) && gt_lt(get_y(point_m), std::minmax(ay, by));
    }
    ...
    PointLike auto&& point_b) noexcept
{
    ...
    return (distance(point_m, point_a) + distance(point_m, point_b) - distance(point_a, point_b)) <= float_cmp_tolerance;
}
