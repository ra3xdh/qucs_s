    ...
    : QDialog(parent)
{
    ...
    */
    ...
    AddSPBlock = new QCheckBox(tr("Add S-Parameter simulation")); // The user can choose
...
void MatchDialog::slotChangeMode_TopoCombo()
{
    ...
    if ((TopoCombo->currentIndex() != 0) && (TopoCombo->currentIndex() != 4)) // Obviously, the microstrip
    ...
    if ((TopoCombo->currentIndex() == 4) || (TopoCombo->currentIndex() == 3)) // Cascaded LC sections selected
    {
...
void MatchDialog::slotButtCreate()
{
    ...
    bool open_short = OpenRadioButton
    ...
    bool success = true;
    double gamma_MAX = MaxRippleEdit->text()
    ...
    if (TwoCheck->isChecked()) { // two-port matching ?
        ...
        double DetReal = S11real * S22real - S11imag * S22imag - S12real * S21real + S12imag * S21imag;
        double DetImag = S11real * S22imag + S11imag * S22real - S12real * S21imag - S12imag * S21real;
        ...
        if ((K > 1) && (delta < 1)) {
            ...
            success = calc2PortMatch(S11real, S11imag, S22real, S22imag, DetReal, DetImag, Z1,
    ...
    double gamma_MAX, bool BalancedStubs)
{
    ...
    case 0: // LC
        ...
        break;
    case 1: // Single stub
        laddercode = calcSingleStub(S11real, S11imag, Z0, Freq, open_short, BalancedStubs);
        break;
    case 2: // Double stub
        laddercode = calcDoubleStub(S11real, S11imag, Z0, Freq, open_short, BalancedStubs);
        break;
    case 3: // Quarter wave cascaded sections
        (BinRadio->isChecked())
            ? laddercode = calcBinomialLines(S11real, S11imag, Z0, order, Freq)
            : laddercode = calcChebyLines(S11real, S11imag, Z0, gamma_MAX, order, Freq);
        break;
    case 4: // Cascaded LC sections
        laddercode = calcMatchingCascadedLCSections(S11real, S11imag, Z0, Freq, order - 1);
        break;
...
QString MatchDialog::calcBiMatch(double S11real, double S11imag, double S22real,
    ...
    bool BalancedStubs)
{
    double B = 1.0 + S11real * S11real + S11imag * S11imag - S22real * S22real - S22imag * S22imag - DetReal * DetReal - DetImag * DetImag;
    ...
    case 3: // Quarter wave cascaded sections
        (BinRadio->isChecked())
            ...
            : laddercode = calcChebyLines(Rreal, -Rimag, Z0, gamma_MAX, order, Freq);
        break;
    case 4: // Cascaded LC sections
    ...
    double gamma_MAX, bool BalancedStubs)
{
    ...
    QString InputLadderCode = calcBiMatch(S11real, S11imag, S22real, S22imag, DetReal, DetImag, Z1,
        Freq, open_short, gamma_MAX, order, BalancedStubs);
    if (InputLadderCode.isEmpty())
        return false; // Synthesis error

    // Output port network
    QString OutputLadderCode = calcBiMatch(S22real, S22imag, S11real, S11imag, DetReal, DetImag, Z2,
        Freq, open_short, gamma_MAX, order, BalancedStubs);
    ...
    double& er_eff, double& zl)
{
    ...
    a = 1.0 + 1.275 * (1.0 - exp(-0.004625 * a * pow(er, 1.674) * pow(freq / 18.365, 2.745))); // = R8
    ...
    tSubstrate Substrate, bool microsyn)
{
    ...
    for (int i = 0; i < strlist.count(); i++) {
        ...
        } else if (!tag.compare("DEV")) // Device painting
        {
            ...
            paintingstr += QStringLiteral("<Rectangle %1 -160 90 50 #000000 0 1 #c0c0c0 1 0>\n")
        ...
        } else if (!tag.compare("TL")) // Transmission line
        {
            if (microsyn) // Microstrip implementation
            {
                ...
                QString val_width = misc::num2str(width, 3, "m"); // Add prefix, unit - 3 significant digits
                QString val_length = misc::num2str(value2 / sqrt(er), 3, "m"); // Add prefix, unit - 3 significant digits
                componentstr += QStringLiteral("<MLIN MS1 1 %3 -120 -26 20 0 0 \"Sub1\" 1 \"%1\" 1 \"%2\" "
        ...
        } else if (!tag.compare("OU")) // Open stub (upper)
        {
            if (microsyn) // Microstrip implementation
            {
                ...
                componentstr += QStringLiteral("<MLIN MS1 1 %3 -180 30 -30 0 1 \"Sub1\" 1 \"%1\" 1 \"%2\" "
                                               "1 \"Hammerstad\" 0 \"Kirschning\" 0 \"26.85\" 0>\n")
                                    .arg(val_width)
                                    .arg(val_length)
                                    .arg(x_pos);
        ...
        } else if (!tag.compare("OL")) // Open stub (lower)
        {
            if (microsyn) // Microstrip implementation
            {
                ...
                // Add suffix mm, cm - 3 significant digits
                QString val_length = misc::num2str(value2 / sqrt(er), 3, "m");
                componentstr += QStringLiteral("<MLIN MS1 1 %3 -60 -26 30 0 1 \"Sub1\" 1 \"%1\" 1 \"%2\" "
                                               "1 \"Hammerstad\" 0 \"Kirschning\" 0 \"26.85\" 0>\n")
                                    .arg(val_width)
                                    .arg(val_length)
                                    .arg(x_pos);
        ...
        } else if (!tag.compare("SL")) // Short circuited stub (lower)
        {
            if (microsyn) // Microstrip implementation
            {
                ...
                QString val_width = misc::num2str(width, 3, "m");
                QString val_length = misc::num2str(value2 / sqrt(er), 3, "m");
                componentstr += QStringLiteral("<MLIN MS1 1 %3 -60 30 -30 0 1 \"Sub1\" 1 \"%1\" 1 \"%2\" "
                                               "1 \"Hammerstad\" 0 \"Kirschning\" 0 \"26.85\" 0>\n")
                                    .arg(val_width)
                                    .arg(val_length)
                                    .arg(x_pos);
        ...
        } else if (!tag.compare("S2P")) // S-param simulation block
        {
            ...
            componentstr += QStringLiteral("<.SP SP1 1 0 100 0 67 0 0 \"lin\" 1 \"%1\" 1 \"%2\" 1 "
                                           "\"300\" 1 \"no\" 0 \"1\" 0 \"2\" 0>\n")
                                .arg((val_freq_start))
                                .arg((val_freq_stop));
        ...
        } else if (!tag.compare("ZL")) // Complex load
        {
            ...
            if ((RL > 1e-3) && (XL < -1e-3)) // R + C
            {
                ...
                componentstr += QStringLiteral("<C C1 1 %1 -90 15 -26 0 -1 \"%2\" 1 0>\n")
                                    .arg(x_pos)
                                    .arg(val_Cap);
                paintingstr += QStringLiteral("<Text %1 50 12 #000000 0 \"%4-j%5 %2 @ %3\">\n")
                                   .arg(x_pos)
                                   .arg(QChar(0x2126))
            ...
            } else if ((RL < 1e-3) && (XL > 1e-3)) // L
            {
                ...
                componentstr += QString(
                    "<L L1 1 %1 -30 15 -26 0 -1 \"%2\" 1 \"26.85\" 0 \"US\" 0>\n")
                                    ...
                                    .arg(val_Ind);
                wirestr += QStringLiteral("<%1 -60 %1 -120>\n").arg(x_pos); // Vertical wire
                paintingstr += QStringLiteral("<Text %1 50 12 #000000 0 \"j%4 %2 @ %3\">\n")
                                   .arg(x_pos)
                                   .arg(QChar(0x2126))
                                   .arg(misc::num2str(Freq, 3, "Hz"))
                                   .arg(XL);
            ...
            paintingstr += QStringLiteral("<Rectangle %1 -150 200 200 #000000 0 1 #c0c0c0 1 0>\n")
                               .arg(x_pos - 30);
        }
    }
    ...
    // Substrate
    if (microsyn)
        componentstr += QStringLiteral("<SUBST Sub1 1 400 200 -30 24 0 0 \"%1\" 1 \"%2mm\" 1 \"%3um\" "
                                       "1 \"%4\" 1 \"%5\" 1 \"%6\" 1>\n")
                            .arg(Substrate.er)
