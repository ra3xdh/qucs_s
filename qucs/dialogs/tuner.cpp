    ...
    , c(component)
{
    ...
    if ((unit.length() <= 1) && (!pp->Description.isEmpty())) { // It comes with no units... so let's try to find a suitable unit
...
void tunerElement::updateProperty()
{
    ...
    if (QucsSettings.DefaultSimulator != spicecompat::simQucsator && suffix == "M") { // Process Mega suffix
        QString unit;
        double num, fac;
