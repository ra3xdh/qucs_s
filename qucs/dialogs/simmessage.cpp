    ...
    : QDialog(parent)
{
    ...
    DataSet = QDir::toNativeSeparators(Info.path()) + QDir::separator() + Doc->getDataSet();
...
bool SimMessage::startProcess()
{
    ...
    QString txt = tr("Starting new simulation on %1 at %2").arg(QDate::currentDate().toString("ddd dd. MMM yyyy")).arg(QTime::currentTime().toString("hh:mm:ss:zzz"));
    ...
    if (!QucsApp::isTextDocument(DocWidget)) {
        SimPorts = ((Schematic*)DocWidget)->prepareNetlist(Stream, Collect, ErrText);
        if (SimPorts < -5) {
            NetlistFile.close();
...
void SimMessage::startSimulator()
{
    ...
    // Simulate schematic window.
        ...
        for (QStringList::Iterator it = Collect.begin(); it != Collect.end(); ++it) {
            ...
            if ((*it).right(4) != ".lst" && (*it).right(5) != ".vhdl" && (*it).right(4) != ".vhd" && (*it).right(2) != ".v") {
                Stream << *it << '\n';
            }
        }
        Stream << '\n';
...
void SimMessage::slotDisplayMsg()
{
    ...
    if (wasLF) {
        ...
        if (i > 1) {
#ifdef SPEEDUP_PROGRESSBAR
            iProgress = 10 * int(ProgressText.at(i - 2).toLatin1() - '0') + int(ProgressText.at(i - 1).toLatin1() - '0');
            if (!waitForUpdate) {
            ...
            int value = 10 * int(ProgressText.at(i - 2).toLatin1() - '0') + int(ProgressText.at(i - 1).toLatin1() - '0');
...
void SimMessage::FinishSimulation(int Status)
{
    ...
    if (Status == 0) {
        QString txt = tr("Simulation ended on %1 at %2").arg(d.toString("ddd dd. MMM yyyy")).arg(t.toString("hh:mm:ss:zzz"));
        ProgText->appendPlainText("\n" + txt + "\n" + tr("Ready."));
    } else {
        QString txt = tr("Errors occurred during simulation on %1 at %2").arg(d.toString("ddd dd. MMM yyyy")).arg(t.toString("hh:mm:ss:zzz"));
        ProgText->appendPlainText("\n" + txt + "\n" + tr("Aborted."));
    }
