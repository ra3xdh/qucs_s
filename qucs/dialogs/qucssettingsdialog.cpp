...
auto getFontDescription = [](const auto& Font) -> QString {
    const QChar comma(u',');
    QString fontDescription = Font.family() + comma + QString::number(Font.pointSize());
...
void QucsSettingsDialog::slotApply()
{
    ...
    QucsSettings.Language = LanguageCombo->currentText().section('(', 1, 1).remove(')');
...
{
    QString d = QFileDialog::getExistingDirectory(this, tr("Select the home directory"),
        homeEdit->text(),
...
{
    QString d = QFileDialog::getExistingDirectory(this, tr("Select the admsXml bin directory"),
        admsXmlEdit->text(),
        QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
...
{
    QString d = QFileDialog::getExistingDirectory(this, tr("Select the ASCO bin directory"),
        ascoEdit->text(),
        QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
...
{
    QString d = QFileDialog::getExistingDirectory(this, tr("Select a directory"),
...
void QucsSettingsDialog::slotAddPathWithSubFolders()
{
    ...
    if (!d.isEmpty()) {
        ...
        while (pathIter.hasNext()) {
            ...
            if (pathfinfo.isDir() && !pathfinfo.isSymLink() && pathIter.fileName() != "." && pathIter.fileName() != "..") {
                QDir thispath(path);
