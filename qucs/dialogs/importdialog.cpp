...
void ImportDialog::slotBrowse()
{
    QString s = QFileDialog::getOpenFileName(
        ...
        tr("All known") + " (*.s?p *.csv *.citi *.cit *.asc *.mdl *.vcd *.dat *.cir *.dat.ngspice *.dat.xyce *.dat.spopus);;" + tr("Touchstone files") + " (*.s?p);;" + tr("CSV files") + " (*.csv);;" + tr("CITI files") + " (*.citi *.cit);;" + tr("ZVR ASCII files") + " (*.asc);;" + tr("IC-CAP model files") + " (*.mdl);;" + tr("VCD files") + " (*.vcd);;" + tr("Qucs dataset files") + " (*.dat *.dat.ngspice *.dat.xyce *.dat.spopus);;" + tr("SPICE files") + " (*.cir);;" + tr("Any file") + " (*)");
...
void ImportDialog::slotSaveBrowse()
{
    QString s = QFileDialog::getSaveFileName(
        this, tr("Enter a Data File Name"),
        lastImportDir.isEmpty() ? QStringLiteral(".") : lastImportDir,
        tr("All known") + " (*.s?p *.csv *.dat *.cir *.net *.lib);;" + tr("Touchstone files") + " (*.s?p);;" + tr("CSV files") + " (*.csv);;" + tr("Qucs dataset files") + " (*.dat);;" + tr("SPICE files") + " (*.cir);;" + tr("Qucsator netlist") + " (*.net);;" + tr("Qucs library") + " (*.lib);;" + tr("Any file") + " (*)");
...
void ImportDialog::slotValidateInput()
{
    ...
    if ((Suffix == "citi") || (Suffix == "cit")) {
        ...
        idx = 2;
    } else if (Suffix == "dat" || FullSuffix == "dat.ngspice" || FullSuffix == "dat.xyce" || FullSuffix == "dat.spopus") {
        idx = 3;
