...
void AbstractSpiceKernel::convertToQucsData(const QString& qucs_dataset)
{
    ...
    for (const QString& ngspice_output_filename : a_output_files) { // For every simulation convert results to Qucs dataset
        ...
        } else if (ngspice_output_filename.endsWith(".four") || four_rx.match(ngspice_output_filename).hasMatch()) {
            isComplex = false;
        ...
        if (hasParSweep) {
            ...
            if (hasDblParSweep && swp_var2_val.isEmpty())
                continue;
            if (hasDblParSweep)
                indep_cnt = sim_points.count() / (swp_var_val.count() * swp_var2_val.count());
            else
