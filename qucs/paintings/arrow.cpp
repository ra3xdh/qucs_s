...
Arrow::Arrow()
    : headStyle(ArrowHeadStyle::empty)
    , headHeight(20.0)
    , headWidth(8.0)
...
QString Arrow::saveCpp()
{
    ...
    QString s = QString("new Line (%1, %2, %3, %4, QPen (QColor (\"%5\"), %6, %7))").arg(x1).arg(y1).arg(x2 - x1).arg(y2 - y1).arg(pen.color().name()).arg(pen.width()).arg(toPenString(pen.style()));
    s = "Lines.append (" + s + ");";
    return s;
}
...
{
    // arrow not allowed in symbols, thus we use line here
    QString s = QStringLiteral("{\"type\" : \"arrow\", "
                               "\"x1\" : %1, \"y1\" : %2, \"x2\" : %3, \"y2\" : %4, "
...
bool Arrow::getSelected(const QPoint& click, int tolerance)
{
    ...
    return qucs_s::geom::is_near_line(click, QPoint(x1, y1), arrowTip, tolerance) || qucs_s::geom::is_near_line(click, headLeftWing, arrowTip, tolerance) || qucs_s::geom::is_near_line(click, headRightWing, arrowTip, tolerance);
}
