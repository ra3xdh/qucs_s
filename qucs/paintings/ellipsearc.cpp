...
{
    QString s = Name + QString::number(x1) + " " + QString::number(y1) + " " + QString::number(x2 - x1) + " " + QString::number(y2 - y1) + " " + QString::number(arcStartAngle) + " " + QString::number(arcLengthAngle) + " " + pen.color().name() + " " + QString::number(pen.width()) + " " + QString::number(pen.style());
    return s;
}
...
{
    QString s = QString("{\"type\" : \"ellipsearc\", "
                        "\"x\" : %1, \"y\" : %2, \"w\" : %3, \"h\" : %4, "
                        "\"angle\" : %5, \"arclen\" : %6, "
...
QPoint calculatePointOnEllipse(const QRect& ellipseBounds, const QPoint& cursorPos)
{
    ...
    return ellipseBounds.center() + QPoint((ellipseBounds.width() / 2.0) * std::cos(angle), (ellipseBounds.height() / 2.0) * std::sin(angle));
}
}
