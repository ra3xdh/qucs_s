...
QString SymbolWidget::theModel()
{
    ...
    if (!ModelString.isEmpty())
        if ((ModelString.count('\n') < 2) || (ModelString.startsWith("<SpiceModel ")) || (ModelString.startsWith("<SpLib ")))
            return ModelString.remove('\n');

    // real library component
    return "<Lib " + Prefix + " 1 0 0 " + QString::number(Text_x) + " " + QString::number(Text_y) + " 0 0 \"" + LibraryPath + "\" 0 \"" + ComponentName + "\" 0>";
}
...
int SymbolWidget::createStandardSymbol(const QString& Lib_, const QString& Comp_)
{
    ...
    if (Comp == "_BJT") {
        ...
        if ((ModelString.section('"', 3, 3).trimmed().at(0) == '-') == (FirstProp == "nfet"))
