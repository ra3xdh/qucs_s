    ...
    : a_netlist2Console(netlist2Console)
{
    ...
    QucsFileFilter = tr("Schematic") + " (*.sch);;" + tr("Symbol only") + " (*.sym);;" + tr("Data Display") + " (*.dpl);;" + tr("Qucs Documents") + " (*.sch *.dpl);;" + tr("VHDL Sources") + " (*.vhdl *.vhd);;" + tr("Verilog Sources") + " (*.v);;" + tr("Verilog-A Sources") + " (*.va);;" + tr("Octave Scripts") + " (*.m *.oct);;" + tr("Spice Files") + QStringLiteral(" (") + QucsSettings.spiceExtensions.join(" ") + QStringLiteral(");;") + tr("Any File") + " (*)";
...
bool QucsApp::gotoPage(const QString& Name)
{
    ...
    QFileInfo Info(Name);
    bool is_sch = false;
    if (Info.suffix() == "sch" || Info.suffix() == "dpl" || Info.suffix() == "sym") {
...
void QucsApp::slotFileExamples()
{
    ...
    auto exampleFile = QFileDialog::getOpenFileName(this, tr("Select example schematic"),
...
void QucsApp::slotSimulate(QWidget* w)
{
    ...
    if (!sim->startProcess()) {
        if (TuningMode == true)
            sim->show(); // The message window is hidden when the tuning mode is active, but in case of error such window pops up
        return;
    }
...
void QucsApp::slotAfterSimulation(int Status, SimMessage* sim)
{
    ...
    if (sim->showBias == 0) { // paint dc bias into schematic ?
        ...
        if (sim->SimOpenDpl) {
            ...
            if (sim->DataDisplay.right(2) == ".m" || sim->DataDisplay.right(4) == ".oct") { // Is it an Octave script?
                octave->startOctave();
...
void QucsApp::slotChangePage(const QString& DocName, const QString& DataDisplay)
{
    ...
    if (d)
        ...
        int i = 0;
        if (ext != "vhd" && ext != "vhdl" && ext != "v" && ext != "va" && ext != "oct" && ext != "m") {
            d = new Schematic(this, Name);
...
void QucsApp::slotOpenContent(const QModelIndex& idx)
{
    ...
    if ((QApplication::keyboardModifiers() & (Qt::ControlModifier | Qt::ShiftModifier)) || (sender() == ActionCMenuOpen && selectionModel->hasSelection())) {
...
void QucsApp::openFileFromProjectView(const QFileInfo& Info, const QString& note)
{
    ...
    if (extName == "sch" || extName == "dpl" || extName == "vhdl" || extName == "vhd" || extName == "v" || extName == "va" || extName == "m" || extName == "oct" || extName == "net") {
    ...
    while (it != QucsSettings.FileTypes.constEnd()) {
        if (extName == (*it).section('/', 0, 0)) {
            ...
            if (Program->state() != QProcess::Running && Program->state() != QProcess::Starting) {
                QMessageBox::critical(this, tr("Error"),
