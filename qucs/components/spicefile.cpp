...
#define HALFWIDTH 17
    ...
    while (i < No) { // add ports lines and numbers
        ...
        if (i == No)
            break; // if odd number of ports there will be one port less on the right side
...
bool SpiceFile::createSubNetlist(QTextStream* stream)
{
    ...
    if (FileName.isEmpty()) {
        ErrText += QObject::tr("ERROR: No file name in SPICE component \"%1\".").arg(Name);
        return false;
    }
    ...
    if (!SpiceFile.open(QIODevice::ReadOnly)) {
        ErrText += QObject::tr("ERROR: Cannot open SPICE file \"%1\".").arg(FileName);
        return false;
    }
    SpiceFile.close();
    ...
    if (changed || !ConvFile.exists() || (lastLoaded.isValid() && lastLoaded < Info.lastModified())) {
        if (!ConvFile.open(QIODevice::WriteOnly)) {
            ErrText += QObject::tr("ERROR: Cannot save converted SPICE file \"%1\".").arg(FileName + ".lst");
            return false;
        }
        outstream = stream;
    ...
    if (!ConvFile.open(QIODevice::ReadOnly)) {
        ErrText += QObject::tr("ERROR: Cannot open converted SPICE file \"%1\".").arg(FileName + ".lst");
        return false;
    }
...
bool SpiceFile::recreateSubNetlist(QString* SpiceFile, QString* FileName)
{
    ...
    if (preprocessor != "none") {
        ...
        if (piping) {
            ...
            if (!PrepFile.open(QIODevice::WriteOnly)) {
                ErrText += QObject::tr("ERROR: Cannot save preprocessed SPICE file \"%1\".").arg(PrepName);
                return false;
            }
        ...
        if (SpicePrep->state() != QProcess::Running && SpicePrep->state() != QProcess::Starting) {
            ErrText += QObject::tr("ERROR: Cannot execute \"%1\".").arg(interpreter + " " + script.join(" ") + "\".");
            if (piping) {
                PrepFile.close();
                delete prestream;
            }
    ...
    if (QucsConv->state() != QProcess::Running && QucsConv->state() != QProcess::Starting) {
