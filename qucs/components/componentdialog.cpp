...
{
    for (auto it = sweepParamWidget.keyValueBegin(); it != sweepParamWidget.keyValueEnd(); ++it) {
        it->second->setLabel(sweepTypeSpecialLabels.contains(qMakePair(type, it->first)) ? sweepTypeSpecialLabels[qMakePair(type, it->first)] : it->second->defaultLabel());
        it->second->setHidden(paramsHiddenBySim.contains(it->first) && paramsHiddenBySim[it->first].contains(component->Model));
        it->second->setEnabled(sweepTypeEnabledParams.contains(type) && sweepTypeEnabledParams[type].contains(it->first));
    }
}
...
void ComponentDialog::slotApplyButton()
{
    ...
    if (isEquation)
        ...
        for (Property* property : component->Props) {
            ...
            /* TODO: ***HACK*** to be fixed */
            if (property->Name == "Symbol" && !excludeList.contains(component->Model))
                continue;
...
void ComponentDialog::slotBrowseFile(QLineEdit* lineEdit)
{
    ...
    if (!s.isEmpty()) {
        ...
        if (!(schematicFileName.isEmpty() && QucsMain->ProjName.isEmpty())) {
            ...
            } else if (QucsSettings.QucsWorkDir.exists(file.fileName()) && QucsSettings.QucsWorkDir.absolutePath() == file.absolutePath()) {
                s = file.fileName();
            }
        }
...
QStringList ComponentDialog::getSimulationList(bool includeGeneric)
{
    ...
    for (Component* c : sch->a_DocComps) {
        ...
        if (c->Model == ".SENS_AC")
            continue;
        if (c->Model == ".SW" && !c->Props.at(0)->Value.toUpper().startsWith("DC"))
            continue;
