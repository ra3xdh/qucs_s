...
RectangularPlotWidget::RectangularPlotWidget(QWidget* parent)
    : QWidget(parent)
    , showTraceValues(true)
    , axisSettingsLocked(false)
    , fMin(1e20)
    , fMax(-1)
{
...
void RectangularPlotWidget::addTrace(const QString& name, const Trace& trace)
{
    ...
    if (!axisSettingsLocked && !traceCopy.trace.isEmpty() && y_autoscale) {
        ...
        if (traceCopy.y_axis == 2) {
            ...
            if (traceMin < y2AxisMin->value() || traceMax > y2AxisMax->value() || (getY2AxisTraceCount() == 1 && traces.size() == 1)) {
            ...
            if (traceMin < yAxisMin->value() || traceMax > yAxisMax->value() || (getYAxisTraceCount() == 1 && traces.size() == 1)) {
...
bool RectangularPlotWidget::addMarker(const QString& markerId, double frequency, const QPen& pen)
{
    ...
    for (auto it = traces.constBegin(); it != traces.constEnd(); ++it) {
        ...
        if (!trace.frequencies.isEmpty() && frequency >= trace.frequencies.first() && frequency <= trace.frequencies.last()) {
...
void RectangularPlotWidget::updatePlot()
{
    ...
    for (auto it = markers.constBegin(); it != markers.constEnd(); ++it) {
        ...
        markerLabel->setHtml("<div style='text-align:center; background:white; padding:2px; border:1px solid " + marker.pen.color().name() + "'>" + freqText + "</div>");
        ...
        for (auto traceIt = traces.constBegin(); traceIt != traces.constEnd(); ++traceIt) {
            ...
            if (!trace.frequencies.isEmpty() && marker.frequency >= trace.frequencies.first() && marker.frequency <= trace.frequencies.last()) {
            ...
            if (found) {
                ...
                if (showTraceValues) {
                    ...
                    valueLabel->setHtml("<div style='background:white; padding:1px; border:1px solid " + trace.pen.color().name() + "'>" + valueText + "</div>");
...
bool RectangularPlotWidget::updateMarkerFrequency(const QString& markerId, double newFrequency)
{
    ...
    for (auto it = traces.constBegin(); it != traces.constEnd(); ++it) {
        const Trace& trace = it.value();
        if (!trace.frequencies.isEmpty() && newFrequency >= trace.frequencies.first() && newFrequency <= trace.frequencies.last()) {
            frequencyInRange = true;
            break;
        }
    }
