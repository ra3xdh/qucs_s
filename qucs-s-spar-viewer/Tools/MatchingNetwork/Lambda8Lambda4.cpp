...
void Lambda8Lambda4::synthesize() {
  ...
  if (Specs.TL_implementation == TransmissionLineType::Ideal) {
    ...
    if (XL != 0) {
      ...
      // It's needed as long as the load is complex to correct the imaginary
      // part
    ...
    if (XL != 0) {
      ...
      // Add its properties
      MStep.val["W1"] = TL1.val["Width"]; // Last microstrip line props are
                                          // still in TL variable
      ...
      // The second width is now known. Add it to the MStep and attach it to the
      // Schematic object
