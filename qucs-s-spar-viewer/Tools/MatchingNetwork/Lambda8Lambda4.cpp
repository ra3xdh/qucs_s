...
void Lambda8Lambda4::synthesize() {
  ...
  MicrostripClass MSL; // Object for synthesizing microstrip lines
  ...
  if (Specs.TL_implementation == TransmissionLineType::Ideal){
    ...
    TL1.setParams(QString("TLIN%1").arg(++Schematic.NumberComponents[TransmissionLine]), TransmissionLine, -90, x_pos, 0);
    ...
    TL1.setParams(QString("MLIN%1").arg(++Schematic.NumberComponents[MicrostripLine]), MicrostripLine, -90, x_pos, 0);
    ...
    if (XL != 0) {
      ...
      MStep.Coordinates = {static_cast<double>(x_pos), 0};
             ...
             // Add its properties
      MStep.val["W1"] = TL1.val["Width"]; // Last microstrip line props are still in TL variable
      ...
      x_pos += 60; // Advance the x-axis drawing index
      ...
      double MS_Length = MSL.Results.length*1e-3;
             ...
             // Instantiate component
      TL2.setParams(QString("MLIN%1").arg(++Schematic.NumberComponents[MicrostripLine]), MicrostripLine, -90, x_pos, 0);
      ...
      // The second width is now known. Add it to the MStep and attach it to the Schematic object
