...
void AttenuatorDesigner::QW_SeriesAttenuator() {
  ...
  // Zout calculation
  double Zout = (R * R * Specs.Zin + 2 * R * Specs.Zin * Specs.Zin) /
                (R * R + 2 * R * Specs.Zin + 2 * Specs.Zin * Specs.Zin);

  // Dispatch to appropriate implementation
  if (Specs.TL_implementation == TransmissionLineType::Lumped) {
    buildAttenuator_QW_Series_Lumped(R, Zout);
  } else {
    if (Specs.TL_implementation == TransmissionLineType::Ideal) {
      buildAttenuator_QW_Series_IdealTL(R, l4, Zout);
    } else if (Specs.TL_implementation == TransmissionLineType::MLIN) {
      buildAttenuator_QW_Series_Microstrip(R, l4, Zout);
    }
  }
}

void AttenuatorDesigner::buildAttenuator_QW_Series_Lumped(double R,
                                                          double Zout) {
...
void AttenuatorDesigner::buildAttenuator_QW_Series_IdealTL(double R, double l4,
                                                           double Zout) {
...
void AttenuatorDesigner::buildAttenuator_QW_Series_Microstrip(double R,
                                                              double l4,
                                                              double Zout) {
  ComponentInfo Ground, Res1, Res2, Res3, TL;
  NodeInfo NI;
