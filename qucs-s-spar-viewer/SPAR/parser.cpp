...
// Being the netlist save in currentNetlist, this function reads the netlist line by line
...
bool SParameterCalculator::parseNetlist() {
  ...
  for (const QString& line : qAsConst(lines)) {
    ...
    if (type == QString("R") && parts.size() >= 4) {
      ...
      addComponent(ComponentType_SPAR::MICROSTRIP_VIA, name.toStdString(), {node1}, value);
