...
void SParameterCalculator::addMicrostripCoupledLinesToAdmittance(vector<vector<Complex>>& Y, const Component_SPAR& comp) {
  ...
  if (node1 > 0) {
    ...
    if (node2 > 0) Y[node1-1][node2-1] += y2;
    if (node3 > 0) Y[node1-1][node3-1] += y3;
    if (node4 > 0) Y[node1-1][node4-1] += y4;
  }

  if (node2 > 0) {
    if (node1 > 0) Y[node2-1][node1-1] += y2;
    Y[node2-1][node2-1] += y1;
    if (node3 > 0) Y[node2-1][node3-1] += y4;
    if (node4 > 0) Y[node2-1][node4-1] += y3;
  }

  if (node3 > 0) {
    if (node1 > 0) Y[node3-1][node1-1] += y3;
    if (node2 > 0) Y[node3-1][node2-1] += y4;
    Y[node3-1][node3-1] += y1;
    if (node4 > 0) Y[node3-1][node4-1] += y2;
  }

  if (node4 > 0) {
    if (node1 > 0) Y[node4-1][node1-1] += y4;
    if (node2 > 0) Y[node4-1][node2-1] += y3;
    if (node3 > 0) Y[node4-1][node3-1] += y2;
...
void SParameterCalculator::calcMicrostripCoupledPropagation(
  ...
  analyseQuasiStaticCoupled(W, h, S, t, er, SModel,
                            ZlEff_e, ZlEff_o, ErEff_e, ErEff_o);

         // Analyse dispersion for even and odd modes
  analyseDispersionCoupled(W, h, S, t, er, ZlEff_e, ZlEff_o, ErEff_e, ErEff_o,
                           frequency, DModel,
                           ZlEffFreq_e, ZlEffFreq_o, ErEffFreq_e, ErEffFreq_o);

         // Analyse losses for even mode
  analyseLoss(W, t, er, rho, 0.0, tand, ZlEff_e, ZlEff_o, ErEff_e,
                    frequency, "Hammerstad", ac_e, ad_e);

         // Analyse losses for odd mode
  analyseLoss(W, t, er, rho, 0.0, tand, ZlEff_o, ZlEff_e, ErEff_o,
                    frequency, "Hammerstad", ac_o, ad_o);
...
void SParameterCalculator::analyseQuasiStaticCoupled(double W, double h, double s, double t, double er, const string& Model,
  ...
  if (Model == "Hammerstad") {
    ...
    r = 1.0 + 0.15 * (1.0 - exp(1.0 - pow(er - 1.0, 2.0) / 8.2) / (1.0 + pow(g, -6.0)));
...
void SParameterCalculator::analyseDispersionCoupled(
    double W, double h, double s, double t, double er,
    double Zle, double Zlo, double ErEffe, double ErEffo,
    double frequency, const string& DModel,
 ...
 if (DModel == "Kirschning") {
    ...
    q15 = 1.887 * exp(-1.5 * pow(g, 0.84)) * pow(g, q14) /
          (1.0 + 0.41 * pow(fn / 15.0, 3.0) *
                     pow(u, 2.0 / q13) / (0.125 + pow(u, 1.626 / q13)));
    ...
    q19 = 0.21 * pow(g, 4.0) / (1.0 + 0.18 * pow(g, 4.9)) / (1.0 + 0.1 * pow(u, 2.0)) /
          (1.0 + pow(fn / 24.0, 3.0));
    q20 = q19 * (0.09 + 1.0 / (1.0 + 0.1 * pow(er - 1.0, 2.7)));
    t = pow(u, 2.5);
    q21 = fabs(1.0 - 42.54 * pow(g, 0.133) * exp(-0.812 * g) * t /
                         (1.0 + 0.033 * t));
    ...
    de = 5.086 * qe * re / (0.3838 + 0.386 * qe) *
         exp(-22.2 * pow(u, 1.92)) / (1.0 + 1.2992 * re) * t / (1.0 + 10.0 * t);
    Ce = 1.0 + 1.275 * (1.0 - exp(-0.004625 * pe * pow(er, 1.674) *
    ...
    t = pow(er - 1.0, 2.0);
    q25 = 0.3 * pow(fn, 2.0) / (10.0 + pow(fn, 2.0)) * (1.0 + 2.333 * t / (5.0 + t));
