...
void SParameterCalculator::calcMicrostripPropagation(double W, double L, double h, double er, double t,
  ...
  // Analyse dispersion
  analyseDispersion(W, h, er, ZlEff, ErEff, frequency, DModel, ZlEffFreq, ErEffFreq);

  // Analyse losses
  analyseLoss(W, t, er, rho, 0.0, tand, ZlEff, ZlEff, ErEff, frequency, "Hammerstad", ac, ad);
...
void SParameterCalculator::analyseQuasiStatic(double W, double h, double t, double er, const string& Model,
  ...
  if (Model == "Hammerstad") {
    ...
    if (t != 0) {
      du1 = t / M_PI * log(1 + 4 * euler / t / pow(1.0/tanh(sqrt(6.517 * u)), 2));
    } else {
      du1 = 0;
    }
...
void SParameterCalculator::analyseDispersion(double W, double h, double er, double ZlEff, double ErEff,
...
void SParameterCalculator::Hammerstad_ab(double u, double er, double& a, double& b) {
  a = 1 + log((pow(u, 4) + pow(u / 52, 2)) / (pow(u, 4) + 0.432)) / 49 +
      log(1 + pow(u / 18.1, 3)) / 18.7;
  b = 0.564 * pow((er - 0.9) / (er + 3), 0.053);
}

void SParameterCalculator::Hammerstad_er(double u, double er, double a, double b, double& e) {
...
void SParameterCalculator::Kirschning_er(double u, double fn, double er, double ErEff, double& ErEffFreq) {
...
void SParameterCalculator::Kirschning_zl(double u, double fn, double er, double ErEff, double ErEffFreq,
                                         double ZlEff, double& r17, double& ZlEffFreq) {
  // Simplified implementation - full implementation would include all r1-r17 calculations
