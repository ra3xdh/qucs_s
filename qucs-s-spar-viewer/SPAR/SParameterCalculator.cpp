...
bool SParameterCalculator::parseNetlist() {
    ...
    for (const QString& line : lines) {
        ...
        if (type == QString("R") && parts.size() >= 4) {
            ...
            double Z0e = parseScaledValue(parts[5]);   // Even mode impedance
            double Z0o = parseScaledValue(parts[6]);   // Odd mode impedance
            double Length = parseScaledValue(parts[7]); // Length in mm

            value["Z0e"] = Z0e;
            value["Z0o"] = Z0o;
            value["Length"] = Length;

            // Update numNodes for all 4 nodes
...
vector<vector<Complex>> SParameterCalculator::calculateCoupledLineYMatrix(double Z0e, double Z0o, double length, double freq) {
    ...
    double omega = 2 * M_PI * freq;
    double beta = omega / c;
    double theta = beta * length * 1e-3; // convert mm to m
