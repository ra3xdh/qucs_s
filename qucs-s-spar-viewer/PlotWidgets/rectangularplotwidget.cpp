...
void RectangularPlotWidget::setupPlot()
{
  ...
  connect(plotWidget->xAxis, QOverload<const QCPRange &>::of(&QCPAxis::rangeChanged), this, &RectangularPlotWidget::onXAxisRangeChanged);
  connect(plotWidget->yAxis, QOverload<const QCPRange &>::of(&QCPAxis::rangeChanged), this, &RectangularPlotWidget::onYAxisRangeChanged);
  connect(plotWidget->yAxis2, QOverload<const QCPRange &>::of(&QCPAxis::rangeChanged), this, &RectangularPlotWidget::onY2AxisRangeChanged);
}
...
void RectangularPlotWidget::updateXAxis()
{
  ...
  disconnect(plotWidget->xAxis, QOverload<const QCPRange &>::of(&QCPAxis::rangeChanged),
      this, &RectangularPlotWidget::onXAxisRangeChanged);

  // Set the axis range in display units
  plotWidget->xAxis->setRange(xMin, xMax);

  // Reconnect the signal
  connect(plotWidget->xAxis, QOverload<const QCPRange &>::of(&QCPAxis::rangeChanged),
   this, &RectangularPlotWidget::onXAxisRangeChanged);
...
void RectangularPlotWidget::updateYAxis()
{
  ...
  // Temporarily disconnect the range changed signal to avoid feedback
  disconnect(plotWidget->yAxis, QOverload<const QCPRange &>::of(&QCPAxis::rangeChanged),
      this, &RectangularPlotWidget::onYAxisRangeChanged);

  plotWidget->yAxis->setRange(yMin, yMax);

  // Reconnect the signal
  connect(plotWidget->yAxis, QOverload<const QCPRange &>::of(&QCPAxis::rangeChanged),
   this, &RectangularPlotWidget::onYAxisRangeChanged);
...
void RectangularPlotWidget::updateY2Axis()
{
  ...
  // Temporarily disconnect the range changed signal to avoid feedback
  disconnect(plotWidget->yAxis2, QOverload<const QCPRange &>::of(&QCPAxis::rangeChanged), this, &RectangularPlotWidget::onY2AxisRangeChanged);
...
/*
  FUNCTIONS FOR HANDLING THE AXIS SETUP WIDGETS WHEN THE USER ZOOMS OR PANS THE GRAPH
 */


void RectangularPlotWidget::onXAxisRangeChanged(const QCPRange &range)
{
  // Only update if axis settings are not locked and the change wasn't triggered by our own update
