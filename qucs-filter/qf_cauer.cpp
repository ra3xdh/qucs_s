...
cauer::cauer(qf_float amin, qf_float amax, qf_float fc, qf_float fs,
    ...
    bool is_tee = false)
    : filter(CAUER, type, imp, fc, bw, is_tee)
    , a_(NULL)
{
    if (amin > amax || (amin > 3 || amax < 3) || ((fc > fs && type_ == LOWPASS) || (fc < fs && type_ == HIGHPASS)) || ((type_ == BANDPASS || type_ == BANDSTOP) && std::abs(fs - (fc * fc) / fs) < bw)) {
        return;
    }
