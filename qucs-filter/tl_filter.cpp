...
// synthesis equations doesn't exist.
void TL_Filter::sythesizeCoupledMicrostrip(double zl_e, double zl_o, double freq, tSubstrate* substrate,
    double& width, double& gap, double& er_eff)
{
    ...
    do {
        // compute line parameters
        getCoupledMicrostrip(width, gap, freq, substrate, zl_e_current, zl_o_current, ce, co);
        ...
        dw = width / 100.0;
        getCoupledMicrostrip(width + dw, gap, freq, substrate, zl_e_result1, zl_o_result1, ce, co);

        // compute line parameters
        dg = gap / 100.0;
        getCoupledMicrostrip(width, gap + dg, freq, substrate, zl_e_result2, zl_o_result2, ce, co);
...
double TL_Filter::dispersionKirschning(double er, double Wh, double freq, double& er_eff, double& zl)
{
    double a, b;
...
void TL_Filter::getCoupledMicrostrip(double ms_width, double g, double freq, tSubstrate* substrate,
    double& zl_e, double& zl_o, double& er_eff_e, double& er_eff_o)
{
    ...
    b = 1.0 + 4.069 * b * pow(g, 0.479) * exp(-1.347 * pow(g, 0.595) - 0.17 * pow(g, 2.5)); // = P7
    ...
    b = 1.887 * exp(-1.5 * pow(g, 0.84)) * pow(g, c)
        / (1.0 + 0.41 * pow(fn / 15.0, 3.0) * pow(Wh_e, 2.0 / b) / (0.125 + pow(Wh_e, 1.626 / b))); // = Q15
    ...
    b = 0.21 * g * g * g * g / ((1.0 + 0.18 * pow(g, 4.9)) * (1.0 + 0.1 * Wh_e * Wh_e) * (1.0 + pow(fn / 24.0, 3.0))); // = Q19
    ...
    b = fabs(1.0 - 42.54 * pow(g, 0.133) * exp(-0.812 * g) * pow(Wh_e, 2.5) / (1.0 + 0.033 * pow(Wh_e, 2.5))); // = Q21
    ...
    a = 1.0 + 0.005 * fn * a / ((1.0 + 0.812 * pow(fn / 15.0, 1.9)) * (1.0 + 0.025 * Wh_o * Wh_o)); // = Q23
    zl_o -= d * a;
