...
poly::poly(int o, qf_float k, const qf_float r[])
    : rep(ROOTS)
    , d(o)
    , p(NULL)
{
...
void poly::div(qf_float r, qf_float i)
{
    ...
    if (d == 1) {
        if (((rep == ROOTS) || (rep == BOTH)) && (std::abs(rts[0] - r) < ROOT_TOL) && (std::abs(rts[1]) < ROOT_TOL)) {
            d = 0;
            delete[] rts;
            rts = NULL;
            delete[] p;
...
void smpf(poly& N, poly& D)
{
    ...
    for (i = 0; i < 2 * dN; i += 2) {
        for (j = 0; j < 2 * dD; j += 2) {
            ...
            if ((Ld[j / 2]) && (std::abs(N.rts[i] - D.rts[j]) < ROOT_TOL) && (std::abs(N.rts[i + 1] - D.rts[j + 1]) < ROOT_TOL)) {
