...
int QUCS_Att::Calc(tagATT* ATT)
{
    ...
    if ((ATT->Topology == PI_TYPE) || (ATT->Topology == TEE_TYPE)) {
        if (ATT->Zin > ATT->Zout) {
            Lmin = (2 * ATT->Zin / ATT->Zout) - 1 + 2 * sqrt(ATT->Zin / ATT->Zout * (ATT->Zin / ATT->Zout - 1));
        } else {
            Lmin = (2 * ATT->Zout / ATT->Zin) - 1 + 2 * sqrt(ATT->Zout / ATT->Zin * (ATT->Zout / ATT->Zin - 1));
        }
    ...
    *s += "<Components>\n";
    ...
    case PI_TYPE:
        ...
        *s += QStringLiteral("<Text 160 60 12 #000000 0 \"%1 dB Pi-Type Attenuator\">\n").arg(ATT->Attenuation);
        if (!SP_box) { // If the SP simulation box option is activated, then the input and output ports are attached.
    ...
    case QW_SERIES_TYPE:
        ...
        *s += "<280 0 450 0 \"\" 0 0 0 \"\">\n";
        ...
        if (ATT->useLumped) { // Add extra wiring to connect the shunt capacitors to the main line
        ...
        *s += "<Paintings>\n";
        ...
        if (ATT->useLumped)
            *s += QStringLiteral("<Text 80 -140 12 #000000 0 \"%1 dB @ %2Hz Quarter-Wave series attenuator\">\n").arg(ATT->Attenuation).arg(num2str(ATT->freq));
        else
            *s += QStringLiteral("<Text 80 -120 12 #000000 0 \"%1 dB @ %2Hz Quarter-Wave series attenuator\">\n").arg(ATT->Attenuation).arg(num2str(ATT->freq));
    ...
    case QW_SHUNT_TYPE:
        ...
        *s += "<Paintings>\n";
        ...
        if (ATT->useLumped)
            *s += QStringLiteral("<Text 80 -140 12 #000000 0 \"%1 dB @ %2Hz Quarter-Wave shunt attenuator\">\n").arg(ATT->Attenuation).arg(num2str(ATT->freq));
        else
            *s += QStringLiteral("<Text 80 -120 12 #000000 0 \"%1 dB @ %2Hz Quarter-Wave shunt attenuator\">\n").arg(ATT->Attenuation).arg(num2str(ATT->freq));
