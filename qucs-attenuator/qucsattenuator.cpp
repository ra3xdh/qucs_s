...
{
    if ((ComboTopology->currentIndex() == BRIDGE_TYPE) || (ComboTopology->currentIndex() == REFLECTION_TYPE) || (ComboTopology->currentIndex() == L_PAD_1ST_SERIES) || (ComboTopology->currentIndex() == L_PAD_1ST_SHUNT) || (ComboTopology->currentIndex() == QW_SERIES_TYPE) || (ComboTopology->currentIndex() == QW_SHUNT_TYPE)) {
...
double QucsAttenuator::ConvertPowerUnits(double Pin, QString from_units, QString to_units)
{
    ...
    else if (from_units == QStringLiteral("dB%1V [75%2]").arg(QChar(0xbc, 0x03)).arg(QChar(0xa9, 0x03)))
        Pin = pow(10, (0.1 * Pin - 12)) / 75; // dBuV [75Ohm] -> W
    else if (from_units == QStringLiteral("dB%1V [50%2]").arg(QChar(0xbc, 0x03)).arg(QChar(0xa9, 0x03)))
        Pin = pow(10, (0.1 * Pin - 12)) / 50; // dBuV [50Ohm] -> W
    else if (from_units == QStringLiteral("dBmV [75%2]").arg(QChar(0xa9, 0x03)))
        Pin = pow(10, (0.1 * Pin - 6)) / 75; // dBmV [75Ohm] -> W
    else if (from_units == QStringLiteral("dBmV [50%2]").arg(QChar(0xa9, 0x03)))
    ...
    if (to_units == "dBm")
        return Pin; // Already done
    else if (to_units == QStringLiteral("dB%1V [75%2]").arg(QChar(0xbc, 0x03)).arg(QChar(0xa9, 0x03)))
        Pin += 108.7506126339170004686755011380612925566374910126647878220; // W -> dBuV [75Ohm]
    else if (to_units == QStringLiteral("dB%1V [50%2]").arg(QChar(0xbc, 0x03)).arg(QChar(0xa9, 0x03)))
